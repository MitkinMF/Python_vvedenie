...
def students_func_delete(e_conn, e_cur):
    show_student(e_cur)
    number = int(input(f'Введите ID человека, которого вы хотели бы удалить: '))
...
def students_func_modify(e_conn, e_cur):
    show_student(e_cur)
    number = int(input(f'Введите ID человека, которого вы хотели бы изменить: '))
    lastname = input('Введите фамилию: ')
    firstname = input('Введите имя: ')
    patronymic = input('Введите отчество: ')
    cls_numb = int(input('Введите номер класса: '))
...
def show_student(e_cur):
    ...
    for index in range(len(buf_list)):
        text = f'ID: {buf_list[index][0]}\t{buf_list[index][1]}\t{buf_list[index][2]} '\
            f'\t{buf_list[index][3]}\tКласс: {buf_list[index][4]}'
        print(text)
    logger.exp_txt('Просмотр списка учеников')
